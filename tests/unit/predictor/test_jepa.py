...
def test_jepa_predictor_init():
    n_speakers = 5
    emb_dim = 16
    model = JEPAPredictor(n_speakers=n_speakers, emb_dim=emb_dim)
...
def test_jepa_predictor_forward():
    ...
    context_identity = torch.zeros(batch_size, n_speakers)  # Simplified for test
    context_embedding = torch.randn(batch_size, emb_dim)
    target_identity = torch.zeros(batch_size, n_speakers)  # Simplified for test
